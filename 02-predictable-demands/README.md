...
* ### ``PersistentVolumeClaim``
    ...
    * ``PersistentVolumeClaim``(PVC): A request for storage by a user or application. It is similiar to how a Pod consumes Node resources (CPU/memory). A PVC represents the "demand" for storage.
    ...
    3. **Verify Application is Writing Data**
    ...
    4. **Test Persistence Across Pod Deletion**
...
* ### ``Secret`` for Sensitive Configuration**
    While ``ConfigMaps`` are great for general configuration, they are not suitable for sensitive information like passwords, tokens, or private keys. For this, Kubernetes provides ``Secret`` resource. A ``Secret`` is similiar to a ``ConfigMap`` but is stored and managed more securely by the cluster.

        ⚠️ Note on Secrets and Encoding: Secret data is stored as Base64-encoded strings, not encrypted. Base64 is an encoding scheme, not an encryption method, and can be easily decoded. The "security" of a Secret comes from Kubernetes' access control policies (RBAC) and how it handles the data within the cluster (e.g., storing it in-memory on nodes where possible). For true security, you should use a vaulting solution and other security patterns.
    ...
    In this example, we will:
    1. Create a ``Secret`` to store a dummy database username and password.
    2. Create a Pod with a runtime dependency on this ``Secret``.
    3. The Pod's container will consume the credentials as environment variables.

    You can find the YAML file for this example here: [02-predictable-demands/k8s-files/05-secret-env-vars.yaml](./k8s-files/05-secret-env-vars.yaml)

    #### 🛠 Hands-on
    
    1. **Encode Your Secret Data**

        Before creating the ``Secret``, you must **Base64-encode** your sensitive values.
        ```bash
        #Encode 'admin'
        echo -n 'admin' | base64
        # output: YWRtaW4=
        ...
        You will need to place these encoded values into the ``05-secret-env-vars.yaml``

    2. **Apply Kubernetes Resources**
    
        Create the ``Secret`` and Pod that depends on it.
        ```bash
        kubectl apply -f 02-predictable-demands/k8s-files/05-secret-env-vars.yaml
        ```

    3. **Check the Pod Logs**

        Verify that the application running in the container was able to access the decoded secret values from its environment.
        ```bash
    ...
    4. **Cleanup**
        ```bash
        kubectl delete -f 02-predictable-demands/k8s-files/05-secret-env-vars.yaml
        ```
